...
func TestResourceImportedResourceCreate(t *testing.T) {
	...
	rd := schema.TestResourceDataRaw(t, resourceImportedResource().Schema, map[string]any{
		...
		"resource":      `{"arn":"arn:aws:iam::123:role/ci"}`,
	})
	...
	if rd.Get("resource_type").(string) != "aws-iam-role" {
		t.Errorf("got resource_type %q, want aws-iam-role", rd.Get("resource_type"))
	}
	...
	// The HCL field is `resource_type` but the GraphQL argument stays `resourceTypeId`.
...
func TestResourceImportedResourceCreateRejectsInvalidPayload(t *testing.T) {
	...
	diags := resourceImportedResourceCreate(t.Context(), rd, pc)
	if !diags.HasError() {
		t.Fatal("expected error from invalid JSON in `resource`")
	}
	if len(rec.Requests) != 0 {
		t.Errorf("expected no API calls when resource fails to parse, got %d", len(rec.Requests))
	}
}
...
func TestResourceImportedResourceRead(t *testing.T) {
	...
	diags := resourceImportedResourceRead(t.Context(), rd, pc)
	if diags.HasError() {
		t.Fatalf("unexpected diagnostics: %v", diags)
	}
...
func TestResourceImportedResourceUpdate(t *testing.T) {
	...
	rd := schema.TestResourceDataRaw(t, resourceImportedResource().Schema, map[string]any{
		"name":          "Updated",
		"resource_type": "aws-iam-role",
		"resource":      `{"arn":"arn:aws:iam::123:role/new"}`,
	})
	rd.SetId("res-1")

	diags := resourceImportedResourceUpdate(t.Context(), rd, pc)
	if diags.HasError() {
		t.Fatalf("unexpected diagnostics: %v", diags)
	}
...
func TestResourceImportedResourceSchema(t *testing.T) {
	...
	if res := r.Schema["resource"]; res == nil || !res.Sensitive {
		t.Error("resource should be Sensitive")
	}
}
